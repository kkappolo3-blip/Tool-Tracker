...
router.patch("/logs/:logId", async (req, res) => {
  ...
  const [updated] = await db.update(logsTable).set(updateData).where(and(eq(logsTable.id, logId), eq(logsTable.userId, OWNER_ID))).returning();
...
router.delete("/logs/:logId", async (req, res) => {
  const logId = parseInt(req.params.logId);
  await db.delete(logsTable).where(and(eq(logsTable.id, logId), eq(logsTable.userId, OWNER_ID)));
